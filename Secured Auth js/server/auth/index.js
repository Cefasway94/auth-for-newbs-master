const express = require('express');                     // Express server and Routing
const Joi = require('@hapi/joi');                     // Validation
const bcrypt = require('bcryptjs');            //  Encryption 
const jwt = require('jsonwebtoken');          //   Jwt tojken

const db = require('../db/connection');
const users = db.get('users');
users.createIndex('username', { unique: true });

const router = express.Router();                   // Using Express routing
 
const schema = Joi.object().keys({
  username: Joi.string().alphanum().min(4).max(30).required(),
  password: Joi.string().trim().regex(/^[a-zA-Z0-9]{3,30}$/).min(10).required()
});
